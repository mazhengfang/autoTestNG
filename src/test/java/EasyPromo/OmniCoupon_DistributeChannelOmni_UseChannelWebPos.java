...
import APITestCase.BackOffice.V2.*;
...
@Slf4j
public class OmniCoupon_DistributeChannelOmni_UseChannelWebPos {
    private WriteExcel writeExl;
    ...
    private String ReqByKey= "createDistChlOmni-UseOmniWebpos";
    ...
    private Omni_merchant_coupon Omni_merchant_coupon;
    ...
    APITestCase.EasyPromo.confirmPromotionPlans confirmPromotionPlans;
    ...
    private String webPosBodyNumber = "6";
    ...
    private APITestCase.BackOffice.V2.OfflineMerchantCoupon OfflineMerchantCoupon;
    ...
    public String promotionPageReqHeaderKey = "common";
    private String promotionPageReqBodyPath = "src\\main\\resources\\ConfigInfo\\BackOffice\\PromotionPageRequestBody.json";
    private String promotionPageRequestBodyKey = "PromotionPage_Create_TestBody_"+bodyNumber;
    private String promotionInfoRequestBodyKey = "PromotionPage_Info_TestBody_"+bodyNumber;
    public String pageCode;
    private String MPMclientName = "MPMSTORE";
    private String DCNclientName = "DCN";
    ...
    public void init(String ENV) {
        ...
        confirmPromotionPlans = new confirmPromotionPlans();
    ...
    public void TC001_createStock(){
        Omni_merchant_coupon = new Omni_merchant_coupon();
        httpResponse createResponse = Omni_merchant_coupon.create(config.URL,config.authorization,rhKey,rhPath,ReqByKey,ReqBodyPath);
    ...
    public void TC039_createPromotionPage(){
        httpResponse ppResponse = poPg.create(config.URL,config.authorization,promotionPageReqHeaderKey,promotionPageHeaderConfigPath,stockID,promotionPageRequestBodyKey,promotionPageReqBodyPath);
        JSONObject rsData = ppResponse.getBody(JSONObject.class).getJSONObject("data");
        String rpsCode = ppResponse.getBody(JSONObject.class).getString("code");
        pageCode = rsData.getString("code");
        pageCode = rsData.getString("page_code");
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<BackOffice>营销页接口创建活动";
        config.expRlt = "200";
        config.actRlt = rpsCode;
        config.testData = pageCode;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        assertEquals( config.actRlt, config.expRlt , "营销页接口创建活动失败 返回的信息是:" + ppResponse.getBody(JSONObject.class).toString());
    }

    @Test(dependsOnMethods = "TC039_createPromotionPage")
    public void TC040_promotionInfo(){
        httpResponse rs = poPg.info(config.URL,config.authorization,promotionPageReqHeaderKey,promotionPageHeaderConfigPath,promotionInfoRequestBodyKey,promotionPageReqBodyPath,pageCode);
        String rpsCode = rs.getBody(JSONObject.class).getString("code");
        ...
        String claimableStatus = poPg.claimableStatus(stocks,stockID);
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<BackOffice>营销信息页接口创建活动";
        config.expRlt = "200";
        config.actRlt = rpsCode;
        config.testData = rpsCode;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        assertEquals( config.actRlt, config.expRlt , "营销页接口创建活动失败 返回的信息是:" + rs.getBody(JSONObject.class).toString());

        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<BackOffice>营销页信息返回活动状态claimable-True";
        config.expRlt = "true";
        config.actRlt = claimableStatus;
        config.testData = claimableStatus;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        assertEquals( config.actRlt, config.expRlt , "营销页接口创建活动失败 返回的信息是:" + rs.getBody(JSONObject.class).toString());
    }

    @Test(dependsOnMethods = "TC040_promotionInfo")
    ...
    @Test(dependsOnMethods = "TC040_promotionInfo")
    public void TC041_getStockInitialStatus() throws Exception {
        table = new Table();
        JSONObject qryResult = table.ep_activity(config.DB, config.DBUser,config.DBPassword, couponSQL3 + activityID);
        String activitySts = qryResult.getString("status");
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<DB>校验ActivitiesDetail接口查询活动状态-CREATED";
        config.expRlt = "CREATED";
        config.actRlt = activitySts;
        config.testData = activitySts;
        config.testRltData=config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt, config.testData,config.testRltData);
    ...
    @Test(dependsOnMethods = "TC041_getStockInitialStatus")
    ...
    public void TC009_queryMPMUserCoupon() {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,MPMclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户优惠券接口返回报错");
        couponList = queryUserCoupon.couponList(rs, config.env,  config.facadeURL,MPMclientName,config.personNumber,"AVAILABLE");
    ...
    @Test(dependsOnMethods = {"TC009_queryMPMUserCoupon"}, enabled = true)
    public void TC033_queryDCNUserCoupon() {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,DCNclientName,"AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户优惠券接口返回报错");
        couponList = queryUserCoupon.couponList(rs, config.env,  config.facadeURL,MPMclientName,config.personNumber,"AVAILABLE");
        boolean couponNotEffective = crossFunctions.couponNotEffective(couponList, stockID);
    ...
    @Test(dependsOnMethods = {"TC033_queryDCNUserCoupon"}, enabled = true)
    public void TC010_MPMUserAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env,config.facadeURL,MPMclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
    ...
    @Test(dependsOnMethods = {"TC010_MPMUserAvailableCoupon"}, enabled = true)
    public void TC034_DCNUserAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env,config.facadeURL,DCNclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponNotEffective = crossFunctions.couponNotEffective(rsData, stockID);
    ...
    @Test(dependsOnMethods = {"TC034_DCNUserAvailableCoupon"}, enabled = true)
    ...
    @Test(dependsOnMethods = "TC011_loggedUserReceiveCoupon")
    public void TC042_getStockApproveStatus() throws Exception {
        sleep(20000);
        table = new Table();
        JSONObject qryResult = table.ep_activity(config.DB, config.DBUser,config.DBPassword, couponSQL3 + activityID);
        String activitySts = qryResult.getString("status");
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<DB>校验线上全场券状态-已审核";
        config.expRlt = "APPROVED";
        config.actRlt = activitySts;
        config.testData = activitySts;
        config.testRltData = config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
    ...
    @Test(dependsOnMethods = "TC042_getStockApproveStatus")
    ...
    @Test(dependsOnMethods = "TC013_queryBUAuditApproveStatus")
    public void TC043_getStockActiveStatus() throws Exception {
        table = new Table();
        JSONObject qryResult = table.ep_activity(config.DB, config.DBUser,config.DBPassword, couponSQL3 + activityID);
        String activitySts = qryResult.getString("status");
    ...
    @Test(dependsOnMethods = "TC043_getStockActiveStatus")
    public void test015_MPMQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,MPMclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
    ...
    @Test(dependsOnMethods = {"TC043_getStockActiveStatus"}, enabled = true)
    public void test016_MPMUserAvailableCoupon() throws InterruptedException {
        sleep(90000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,MPMclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "userAvailableCoupon接口返回报错");
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
    ...
    @Test(dependsOnMethods = "TC043_getStockActiveStatus")
    public void test035_DCNQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,DCNclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询queryUserCoupon接口返回报错");
    ...
    @Test(dependsOnMethods = {"TC043_getStockActiveStatus"}, enabled = true)
    public void test036_DCNUserAvailableCoupon() throws InterruptedException {
        sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,DCNclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
        ...
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponEffective = crossFunctions.couponEffective(rsData, stockID);
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<Facade>校验生效活动在userAvailableCoupon展示";
        config.expRlt = String.valueOf(true);
        config.actRlt = String.valueOf(couponEffective);
        config.testRltData =config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt," ",config.testRltData);
        assertEquals(config.actRlt, config.expRlt , "生效活动在PDP页面接口未展示活动数据，期望结果展示");
    }
    ...
    public void test018_MPMQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,MPMclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
    ...
    @Test(dependsOnMethods = {"test017_loggedUserReceiveCoupon"}, enabled = true)
    public void test019_MPMUserAvailableCoupon() throws InterruptedException {
        sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,MPMclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test017_loggedUserReceiveCoupon"}, enabled = true)
    public void test037_DCNQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,DCNclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test017_loggedUserReceiveCoupon"}, enabled = true)
    public void test038_DCNUserAvailableCoupon() throws InterruptedException {
        sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,DCNclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "userAvailableCoupon接口返回报错");
    ...
    public void test020_webPosPlans() throws InterruptedException {
        httpResponse rs = plans.plans(config.env, config.URL, webPosBodyNumber,config.personNumber);
        int rsCode = rs.getCode();
...
//        JSONArray couponsList = rsData.getJSONArray("coupons");
    ...
    @Test(dependsOnMethods = {"test020_webPosPlans"}, enabled = true)
    public void test022_confirmPromotionPlans() throws Exception {
        httpResponse rs = confirmPromotionPlans.confirmPromotionPlans(config.env, config.URL, webPosBodyNumber,planIndex,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "Confirm Promotion Plan接口返回报错");
        orderID = confirmPromotionPlans.orderID;
        table = new Table();
    ...
    @Test(dependsOnMethods = {"test022_confirmPromotionPlans"}, enabled = true)
    public void test023_MPMQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon( config.env, config.facadeURL,MPMclientName, "USED", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
        couponList = queryUserCoupon.couponList(rs, config.env, config.facadeURL,MPMclientName, config.personNumber,"USED");
    ...
    @Test(dependsOnMethods = {"test022_confirmPromotionPlans"}, enabled = true)
    public void test024_MPMUserAvailableCoupon() throws InterruptedException {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon( config.env, config.facadeURL,MPMclientName, bodyNumber, config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponCodeStatus = crossFunctions.couponCodeValidation(rsData, stockID, null,"UNCLAIMED");
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<Facade>校验userAvailableCoupon优惠券状态-UNCLAIMED";
        config.expRlt = String.valueOf(true);
        config.actRlt = String.valueOf(couponCodeStatus);
        config.testRltData =config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt, "UNCLAIMED" ,config.testRltData);
        assertEquals(config.actRlt, config.expRlt, "冻券后优惠券状态异常，期望结果是UNCLAIMED");
    }

    @Test(dependsOnMethods = {"test022_confirmPromotionPlans"}, enabled = true)
    public void test039_DCNQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon( config.env, config.facadeURL,DCNclientName, "USED", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
        couponList = queryUserCoupon.couponList(rs, config.env, config.facadeURL,MPMclientName, config.personNumber,"USED");
        boolean couponCodeStatus = crossFunctions.couponCodeValidation(couponList, stockID, couponCodeByStockID,"USED");
    ...
    @Test(dependsOnMethods = {"test022_confirmPromotionPlans"}, enabled = true)
    public void test040_DCNUserAvailableCoupon() throws InterruptedException {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon( config.env, config.facadeURL,DCNclientName, bodyNumber, config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
    ...
    @Test(dependsOnMethods = {"TC036_H5DistributeChannel"}, enabled = true)
    public void test025_promotionDetails_001() throws InterruptedException {
        httpResponse rs = promotionDetails.promotionDetails(config.URL, orderID, webPosBodyNumber);
        int rsCode = rs.getCode();
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        ...
        JSONObject rsData = rs.getBody(JSONObject.class).getJSONObject("data");
        JSONObject expPromotionDetailsData = parseObject(commonFunction.readJsonFile(PromotionDetailsExpectData)).getJSONObject(promotionDetailsKey + webPosBodyNumber);
...
//                    System.out.println("Key4 " + sKey + ":" + actItemData + ":" + expItemData);
    ...
    public void test026_redeemCoupon() throws Exception {
        httpResponse rs = redeemCoupon.redeemCoupon(config.URL, orderID, webPosBodyNumber);
        int rsCode = rs.getCode();
    ...
    public void test027_refundCoupon() throws Exception {
        httpResponse rs = refundCoupon.refundCoupon(config.URL, orderID, webPosBodyNumber);
        int rsCode = rs.getCode();
    ...
    public void test028_MPMQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,MPMclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询queryUserCoupon接口返回报错");
    ...
    @Test(dependsOnMethods = {"test027_refundCoupon"}, enabled = true)
    public void test029_MPMUserAvailableCoupon() throws InterruptedException {
         sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,MPMclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test027_refundCoupon"}, enabled = true)
    public void test041_DCNQueryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(config.env, config.facadeURL,DCNclientName, "AVAILABLE", "ALL", 1, 5,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test027_refundCoupon"}, enabled = true)
    public void test042_DCNUserAvailableCoupon() throws InterruptedException {
        sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(config.env, config.facadeURL,DCNclientName, bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test030_全场券活动关闭"}, enabled = true)
    public void test032_MPMUserAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon( config.env,config.facadeURL,MPMclientName,bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
    ...
    @Test(dependsOnMethods = {"test030_全场券活动关闭"}, enabled = true)
    public void test043_DCNUserAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon( config.env,config.facadeURL,DCNclientName,bodyNumber,config.personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "指定商品用户拥有的优惠券接口返回报错");
    ...
    @Test(dependsOnMethods = "test030_全场券活动关闭")
    public void TC041_promotionInfo() throws InterruptedException {
        sleep(30000);
        httpResponse rs = poPg.info(config.URL,config.authorization,promotionPageReqHeaderKey,promotionPageHeaderConfigPath,promotionInfoRequestBodyKey,promotionPageReqBodyPath,pageCode);
        JSONObject rsData = rs.getBody(JSONObject.class).getJSONObject("data");
        String rpsCode = rs.getBody(JSONObject.class).getString("code");
        JSONArray stocks =  (rsData.getJSONObject("promotion_info")).getJSONArray("stocks");
        String claimableStatus = poPg.claimableStatus(stocks,stockID);
        String couponStatus = poPg.couponStatus(stocks,stockID);
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<BackOffice>活动关闭,营销页信息接口返回状态";
        config.expRlt = "200";
        config.actRlt = rpsCode;
        config.testData = rpsCode;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        ...
        config.stepDescription = "<BackOffice>活动关闭,营销页信息返回claimable-false";
        config.expRlt = "false";
        config.actRlt = claimableStatus;
        config.testData = claimableStatus;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        assertEquals( config.actRlt, config.expRlt , "营销页信息返回claimable状态失败 返回的信息是:" + rs.getBody(JSONObject.class).toString());


        config.stepDescription = "<BackOffice>活动关闭,营销页信息返回CouponStatus-CLOSED";
        config.expRlt = "CLOSED";
        config.actRlt = couponStatus;
        config.testData = couponStatus;
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,config.testData,config.testRltData);
        assertEquals( config.actRlt, config.expRlt , "营销页信息返回CouponStatus活动失败 返回的信息是:" + rs.getBody(JSONObject.class).toString());
    }


    @AfterTest
    public void report() {
        writeExl.writeExcel("Omni_Omni_WebPos ",config.testRltData,8,config.filePath);
    }

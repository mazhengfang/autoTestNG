...
@Slf4j
public class OnlineCoupon_DistributeChannelMPM_UseChannelMPM {
    ...
    private String ReqByKey= "createDistChlMPM_PPage-UseMPM";
...
//    private String sql_ep_activity_instance = "select * from ep_activity ea where ewi.id =" ;
    ...
    @Test(dependsOnMethods = "TC002_queryStockID")
    public void TC033_getStockInitialStatus() throws Exception {
        table = new Table();
    ...
    @Test(dependsOnMethods = "TC033_getStockInitialStatus")
    ...
    @Test(dependsOnMethods = {"TC010_userAvailableCoupon"}, enabled = true)
    ...
    @Test(dependsOnMethods = "TC011_loggedUserReceiveCoupon")
    public void TC034_getStockApproveStatus() throws Exception {
        table = new Table();
        JSONObject qryResult = table.ep_activity(config.DB, config.DBUser,config.DBPassword, couponSQL3 + activityID);
        String activitySts = qryResult.getString("status");
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
    ...
    @Test(dependsOnMethods = "TC034_getStockApproveStatus")
    ...
    @Test(dependsOnMethods = "TC013_queryBUAuditApproveStatus")
    public void TC035_getStockActiveStatus() throws Exception {
        table = new Table();
        JSONObject qryResult = table.ep_activity(config.DB, config.DBUser,config.DBPassword, couponSQL3 + activityID);
        String activitySts = qryResult.getString("status");
    ...
    @Test(dependsOnMethods = "TC035_getStockActiveStatus")
...
//                    System.out.println("Key4 " + sKey + ":" + actItemData + ":" + expItemData);
    ...
    @AfterTest
    public void report() {
    writeExl.writeExcel("Oln_MPM_MPM ",config.testRltData,8,config.filePath);
    }

...
public class OnlineCoupon_DistributeChannelPromotionPage_UseChannelMPM {
    ...
    private String ReqByKey= "createDistChlPoPg_PPage-UseMPM";
    ...
    public void TC034_H5DistributeChannel() {
        ...
        config.stepName = Thread.currentThread().getStackTrace()[1].getMethodName();
        config.stepDescription = "<Facade>校验生效活动在activityPageAvailableCoupons接口不展示";
        config.expRlt = "true";
        config.actRlt = String.valueOf(couponNotEffective);
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt," ",config.testRltData);
        assertEquals(config.actRlt,config.expRlt, "生效活动H5投放渠道展示活动数据，期望结果不展示");
    }
...
//        JSONArray couponsList = rsData.getJSONArray("coupons");
    ...
    public void test022_applyPromotionPlans() throws Exception {
        ...
        config.rpt.testResultData(config.stepName,config.stepDescription,styType,config.expRlt,config.actRlt,orderID ,config.testRltData);
...
//                    System.out.println("Key4 " + sKey + ":" + actItemData + ":" + expItemData);
    ...
    @AfterTest
    public void report() {
        writeExl.writeExcel("Oln_PP_MPM ",config.testRltData,8,config.filePath);
    }

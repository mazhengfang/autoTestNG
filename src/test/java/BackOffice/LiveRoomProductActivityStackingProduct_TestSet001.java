...
public class LiveRoomProductActivityStackingProduct_TestSet001 {
    private String tcPath = "src/main/resources/ConfigInfo/TestCase/backOfficeTestCaseData.json";
    private String tcKey = "LiveRoomProductActivityStackingProduct";
    ...
    public void test003_创建直播券() throws InterruptedException {
        boolean newCouponResult = createActivity.createLiveRoomProductActivity(driver,"product","[Auto]直叠单",15,"105","5");
        assertEquals(newCouponResult, true, "BackOffice创建直播券失败");
    }

    @Test(dependsOnMethods = {"test003_创建直播券"}, enabled = true)
    public void test004_直播券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"product","product");
    }

    @Test(dependsOnMethods = {"test004_直播券页面获取"}, enabled = true)
    public void test005_校验直播券() {
...
//        assertEquals(activityInfo.get("Stacking Checked"), "true", "Coupon Stacking Status is not correct");
    }

    @Test(dataProvider = "test005_校验直播券", dependsOnMethods = {"test004_直播券页面获取"}, enabled = true)
    public void test006_校验直播券(backOfficeTestCase tc) {
    ...
    @DataProvider(name = "test005_校验直播券")
    private Object[][] test005_校验直播券_TestData() {

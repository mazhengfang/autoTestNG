...
public class TestSet_H5_MPMSTORE_003_DingXiang {
    WebDriver driver;
    ...
    public void test003_创建活动页定向券() throws InterruptedException {
        boolean newCouponResult = createActivity.createH5DesignatedUserActivity(driver,15,"105","5");
        assertEquals(newCouponResult, true, "BackOffice创建定向券失败");
    }

    @Test(dependsOnMethods = {"test003_创建活动页定向券"}, enabled = true)
    public void test004_活动页定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"normal","noStack");
    }

    @Test(dependsOnMethods = {"test004_活动页定向券页面获取"}, enabled = true)
    public void test005_活动页校验定向券() {
    ...
    @Test(dataProvider = "test005_活动页校验定向券", dependsOnMethods = {"test005_活动页校验定向券"}, enabled = true)
    public void test006_活动页校验定向券(backOfficeTestCase tc) {
        String sKey = tc.getKey();
        String sValue = tc.getValue();
        String actualValue = activityInfo.get(sKey);
        System.out.println("活动页创建页面校验：" + sKey+ "- 期望值：" + sValue + "- 实际值："+ actualValue );
        assertEquals(actualValue, sValue, sKey + " value is not correct");
    }

    @DataProvider(name = "test005_活动页校验定向券")
    private Object[][] test005_活动页校验定向券_TestData() {
    ...
    @Test(dependsOnMethods = {"test006_活动页校验定向券"}, enabled = true)
    ...
    public void test009_活动页定向券页面获取() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test009_活动页定向券页面获取"}, enabled = true)
    ...
    public void test014_plans() throws InterruptedException {
        ...
        System.out.println("Activity ID - Plan Index : " + activityInfo.get("Activity ID")+ " - " + planIndex);
    }

...
//    @BeforeSuite
    ...
    public void test008_userAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(Env, facadePromoHost, bodyNumber4,personNumber);
        int rsCode = rs.getCode();
...
//    @Test(dependsOnMethods = {"test026_loggedUserReceiveCoupon"}, enabled = true)
    public void test027_queryUserCoupon() throws InterruptedException {
        sleep(720000);
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(Env, facadePromoHost, "ALL", "ALL", 1, 5,personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
        couponList = queryUserCoupon.couponList(rs, Env, facadePromoHost,personNumber);

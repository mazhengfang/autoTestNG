...
public class TestSet_MPMSTORE_MPMSTORE_002_QuanChang {
    ...
    public void test003_创建全场券() throws InterruptedException {
        boolean newCouponResult = createActivity.createNormalActivity(driver, "[Auto]全场券", "noStack",5,"398","360");
    ...
    public void test014_创建全场券() throws InterruptedException {
        boolean newCouponResult = createActivity.createNormalActivity(driver, "[Auto]全场券", "noStack",3,"398","370");
        assertEquals(newCouponResult, true, "BackOffice创建全场券失败");
    }

    @Test(dependsOnMethods = {"test014_创建全场券"}, enabled = true)
    public void test015_全场券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver, "normal", "noStack");
    }

    @Test(dependsOnMethods = {"test015_全场券页面获取"}, enabled = true)
    public void test016_全场券页面获取() throws InterruptedException {
        activityInfo = validateActivity.UpdateActivity(driver, activityInfo, "normal", "noStack",120000);
        assertEquals(activityInfo.get("Active Status"), "Take effect", "活动状态变更错误");
        System.out.println("Part 2"+activityInfo.toString());
    }

    @Test(dependsOnMethods = {"test016_全场券页面获取"}, enabled = true)

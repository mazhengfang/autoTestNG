...
public class TestSet_MPMSTORE_MPMSTORE_004_DingXiang {
    ...
    public void test003_创建定向券() throws InterruptedException {
        boolean newCouponResult = createActivity.createDesignatedUserActivity(driver,5,"398","360");
    ...
    public void test005_定向券页面获取() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test005_定向券页面获取"}, enabled = true)
    ...
    public void test014_创建定向券() throws InterruptedException {
        boolean newCouponResult = createActivity.createDesignatedUserActivity(driver,3,"398","370");
        assertEquals(newCouponResult, true, "BackOffice创建定向券失败");
    }

    @Test(dependsOnMethods = {"test014_创建定向券"}, enabled = true)
    public void test015_定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"normal","noStack");
    }

    @Test(dependsOnMethods = {"test015_定向券页面获取"}, enabled = true)
    public void test016_定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.UpdateActivity(driver, activityInfo, "normal", "noStack",130000);
        assertEquals(activityInfo.get("Active Status"), "Take effect", "活动状态变更错误");
    }

    @Test(dependsOnMethods = {"test016_定向券页面获取"}, enabled = true)

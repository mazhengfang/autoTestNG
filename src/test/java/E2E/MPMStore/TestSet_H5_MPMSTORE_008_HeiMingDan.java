...
public class TestSet_H5_MPMSTORE_008_HeiMingDan {
    ...
    private String tcKey = "H5BlackModelListNormalActivity";
...
//    @BeforeSuite
    ...
    public void test003_创建活动页黑名单全场券() throws InterruptedException {
        boolean newCouponResult = createActivity.createH5BlackModelListNormalActivity(driver,15,"105","5");
        assertEquals(newCouponResult, true, "BackOffice创建黑名单活动失败");
    }

    @Test(dependsOnMethods = {"test003_创建活动页黑名单全场券"}, enabled = true)
    public void test004_活动页黑名单全场券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"normal","noStack");

        String notApplicableGoodTypePath = "//*[@id=\"pane-first\"]/div/div[6]/form/div/div[9]/div[1]/div/p";
        ...
        String notApplicableGoodIDPath = "//*[@id=\"pane-first\"]/div/div[6]/form/div/div[9]/div[2]/div[3]/table/tbody/tr[1]/td[1]/div";
        ...
        String notApplicableGoodID2Path = "//*[@id=\"pane-first\"]/div/div[6]/form/div/div[9]/div[2]/div[3]/table/tbody/tr[2]/td[1]/div";
        ...
        String notApplicableGoodID3Path = "//*[@id=\"pane-first\"]/div/div[6]/form/div/div[9]/div[2]/div[3]/table/tbody/tr[3]/td[1]/div";
    ...
    @Test(dependsOnMethods = {"test004_活动页黑名单全场券页面获取"}, enabled = true)
    public void test005_校验活动页黑名单全场券() {
    ...
    @Test(dataProvider = "test005_校验活动页黑名单全场券", dependsOnMethods = {"test005_校验活动页黑名单全场券"}, enabled = true)
    public void test006_校验活动页黑名单全场券(backOfficeTestCase tc) {
    ...
    @DataProvider(name = "test005_校验活动页黑名单全场券")
    private Object[][] test005_校验活动页黑名单全场券_TestData() {
    ...
    @Test(dependsOnMethods = {"test006_校验活动页黑名单全场券"}, enabled = true)
    public void test007_queryUserCoupon() {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(Env, facadePromoHost, "MPMSTORE", "AVAILABLE", "ALL", 1, 5,personNumber);
    ...
    public void test008_userAvailableCoupon() {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(Env, facadePromoHost, "MPMSTORE", bodyNumber4,personNumber);
        int rsCode = rs.getCode();
    ...
    public void test031_userAvailableCoupon() {
    ...
    @Test(dependsOnMethods = {"test031_userAvailableCoupon"}, enabled = true)
    ...
    public void test009_活动页全场券页面获取() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test009_活动页全场券页面获取"}, enabled = true)
    ...
    public void test011_userAvailableCoupon() throws InterruptedException {
        ...
        assertEquals(couponNotEffective, false, "生效活动在PDP页面接口未展示活动数据，期望结果不展示");
    }
    ...
    public void test029_userAvailableCoupon() throws InterruptedException {
        ...
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponNotEffective = crossFunctions.couponEffective(rsData, activityInfo.get("StockID"));
        assertEquals(couponNotEffective, false, "生效活动在PDP页面接口未展示活动数据，期望结果展示");
    }
    ...
    public void test034_userAvailableCoupon() throws InterruptedException {
        sleep(10000);
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(Env, facadePromoHost, "MPMSTORE", bodyNumber4,personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "我的优惠券接口返回报错");
        ...
        assertEquals(couponCodeStatus, false, "领券后优惠券状态异常，期望结果是不展示");
    }

    @Test(dependsOnMethods = {"test034_userAvailableCoupon"}, enabled = true)
    public void test030_userAvailableCoupon() throws InterruptedException {
        httpResponse rs = userAvailableCoupon.userAvailableCoupon(Env, facadePromoHost, "MPMSTORE", bodyNumber,personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "我的优惠券接口返回报错");
        ...
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponNotEffective = crossFunctions.couponEffective(rsData, activityInfo.get("StockID"));
        assertEquals(couponNotEffective, true, "生效活动在PDP页面接口未展示活动数据，期望结果展示");
    }

    //A1B2 B是指定黑名单商品 A是非黑名单商品 商品价格未满足门槛
    @Test(dependsOnMethods = {"test030_userAvailableCoupon"}, enabled = true)
    public void test032_plans() throws InterruptedException {
        httpResponse rs = plans.plans(Env, easyPromoHost, bodyNumber3,personNumber);
        int rsCode = rs.getCode();
        ...
        assertEquals(couponsList.size(),1,"未指定商品返回的促销信息，期待结果是不返回促销信息");
    ...
    //B2 B是指定黑名单商品
    @Test(dependsOnMethods = {"test032_plans"}, enabled = true)
    public void test033_plans() throws InterruptedException {
        httpResponse rs = plans.plans(Env, easyPromoHost, bodyNumber4,personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "Plans接口返回报错");
    ...
    //A1B2 B是指定黑名单商品 A是非黑名单商品 商品价格满足门槛
    @Test(dependsOnMethods = {"test033_plans"}, enabled = true)
    public void test014_plans() throws InterruptedException {
        ...
        JSONArray couponsList = rsData.getJSONArray("coupons");
        JSONArray promotionPlansList = rsData.getJSONArray("promotion_plans");
        JSONObject expPromotionPlans = parseObject(commonFunction.readJsonFile(plansExpectData)).getJSONObject(plansPPKey + bodyNumber3);
    ...
    public void test015_repricing() throws InterruptedException {
        ...
        JSONObject expPromotionDetail = parseObject(commonFunction.readJsonFile(repricingPromotionDetailExpectData)).getJSONObject(repricingPromotionDetailKey + bodyNumber3);
...
//        assertEquals(dbRecord.getString("status"), "FROZEN", "Redeem Coupon接口返回报错");
    ...
    public void test020_queryUserCoupon() throws InterruptedException {
        JSONArray couponList;
        httpResponse rs = queryUserCoupon.queryUserCoupon(Env, facadePromoHost, "MPMSTORE", "USED", "ALL", 1, 5,personNumber);
        int rsCode = rs.getCode();
        assertEquals(rsCode, 200, "查询用户领券接口返回报错");
        ...
        assertEquals(couponCodeStatus, true, "冻券后优惠券状态异常，期望结果是USED");
    }
    ...
    public void test017_promotionDetails_001() throws InterruptedException {
        ...
        JSONObject rsData = rs.getBody(JSONObject.class).getJSONObject("data");
        JSONObject expPromotionDetailsData = parseObject(commonFunction.readJsonFile(PromotionDetailsExpectData)).getJSONObject(promotionDetailsKey + bodyNumber3);
...
//        assertEquals(dbRecord.getString("status"), "EXCHANGED", "Refund Coupon接口返回报错");
    ...
    public void test029_活动页黑名单券活动关闭() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test029_活动页黑名单券活动关闭"}, enabled = true)
...
//    @Test(dependsOnMethods = {"test026_loggedUserReceiveCoupon"}, enabled = true)
    public void test027_queryUserCoupon() throws InterruptedException {
        sleep(720000);
        JSONArray couponList;
        ...
        assertEquals(couponCodeStatus, true, "活动关闭后，优惠券状态异常，期望结果是EXPIRED");
    }

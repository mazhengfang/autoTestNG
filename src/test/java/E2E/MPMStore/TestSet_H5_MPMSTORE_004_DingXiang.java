...
public class TestSet_H5_MPMSTORE_004_DingXiang {
    WebDriver driver;
    ...
    private String tcKey = "H5DesignatedUsersActivity";
    ...
    public void test003_创建活动页定向券() throws InterruptedException {
        boolean newCouponResult = createActivity.createH5DesignatedUserActivity(driver,12,"398","360");
        assertEquals(newCouponResult, true, "BackOffice创建定向券失败");
    }

    @Test(dependsOnMethods = {"test003_创建活动页定向券"}, enabled = true)
    public void test004_H5定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"normal","noStack");
    }

    @Test(dependsOnMethods = {"test004_H5定向券页面获取"}, enabled = true)
    public void test005_活动页定向券页面获取() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test005_活动页定向券页面获取"}, enabled = true)
    ...
    @Test(dependsOnMethods = {"test013_创建活动页面"}, enabled = true)
    public void test014_创建活动页定向券() throws InterruptedException {
        boolean newCouponResult = createActivity.createH5DesignatedUserActivity(driver,3,"398","370");
        assertEquals(newCouponResult, true, "BackOffice创建定向券失败");
    }

    @Test(dependsOnMethods = {"test014_创建活动页定向券"}, enabled = true)
    public void test015_活动页定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.validateActivity(driver,"normal","noStack");
    }

    @Test(dependsOnMethods = {"test015_活动页定向券页面获取"}, enabled = true)
    public void test016_活动页定向券页面获取() throws InterruptedException {
        activityInfo = validateActivity.UpdateActivity(driver, activityInfo, "normal", "noStack",130000);
        assertEquals(activityInfo.get("Active Status"), "Take effect", "活动状态变更错误");
    }

    @Test(dependsOnMethods = {"test016_活动页定向券页面获取"}, enabled = true)
    ...
    public void test021_loggedUserReceiveCoupon() throws InterruptedException {
        sleep(420000);
        httpResponse rs = loggedUserReceiveCoupon.loggedUserReceiveCoupon_StockID(Env, facadePromoHost, personNumber,"MPMSTORE", activityInfo.get("StockID"));
        int rsCode = rs.getCode();
    ...
    public void test022_queryDesignatedUserCoupon() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test022_queryDesignatedUserCoupon"}, enabled = true)

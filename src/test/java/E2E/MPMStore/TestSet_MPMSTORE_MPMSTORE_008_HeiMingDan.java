...
public class TestSet_MPMSTORE_MPMSTORE_008_HeiMingDan {
    ...
    public void test003_创建黑名单全场券() throws InterruptedException {
        boolean newCouponResult = createActivity.createBlackModelListNormalActivity(driver,65,"105","5");
    ...
    @Test(dependsOnMethods = {"test006_校验黑名单全场券"}, enabled = true)
    ...
    public void test009_全场券页面获取() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test009_全场券页面获取"}, enabled = true)
...
//        assertEquals(dbRecord.getString("status"), "FROZEN", "Redeem Coupon接口返回报错");
    ...
    public void test021_userAvailableCoupon() throws InterruptedException {
        ...
        assertEquals(rsCode, 200, "我的优惠券接口返回报错");
        JSONArray rsData = rs.getBody(JSONObject.class).getJSONArray("data");
        boolean couponCodeStatus = crossFunctions.couponCodeValidation(rsData,  activityInfo.get("StockID"), null,"UNCLAIMED");
        assertEquals(couponCodeStatus, true, "冻券后优惠券状态异常，期望结果是UNCLAIMED");
    }
...
//        assertEquals(dbRecord.getString("status"), "EXCHANGED", "Refund Coupon接口返回报错");
    ...
    public void test029_黑名单券活动关闭() throws InterruptedException {
    ...
    @Test(dependsOnMethods = {"test029_黑名单券活动关闭"}, enabled = true)
